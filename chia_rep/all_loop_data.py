...
class AllLoopData:
    ...
    def __init__(self, chrom_size_file_path, in_file_path,
                 bedgraph, is_hiseq, wanted_chroms=None, min_loop_value=0):

        log.debug(locals())
        wanted_chroms = [x for x in wanted_chroms if x]
    ...
    def preprocess(self, peak_file_path,
                   peak_percent_kept=DEFAULT_PEAK_PERCENT, window_size=None):
        to_remove = []
        for name, chrom in self.chrom_dict.items():
            if not chrom.preprocess(peak_file_path, peak_percent_kept,
                                    window_size):
    ...
    def compare(self, o_loop_data, bin_size, window_size, window_index=None,
        ...
        chrom_value_list = []
        log.info(f'Chroms to compare: {wanted_chroms}')
